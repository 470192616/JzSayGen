...
namespace JzSayDemo.JM
{
    public partial class UIKeyWordsList : PageBase
    {
        ...
        protected List<KeyWordsLib> ListData = new List<KeyWordsLib>();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                var sql = LinqWhereHelper.True<KeyWordsLib>();
                ...
                {
                    sql = sql.And(x => x.KeyWord.Contains(this.SearchName));
                    attachUrl = attachUrl + "&SearchName=" + this.SearchName;
                }

                var tmp = db.KeyWordsLib.Where(sql);
        ...
        void CmdProcess()
        {
            ...
            switch (cmd)
            {
                case "statehide":
                    {
                        sql = "UPDATE KeyWordsLib SET UpdateTS=@uts, [Stat]=" + EGenStat.Hiden.GetInt32Str() + " WHERE UrlKey=@UK ";
                        break;
                    }
                case "stateshow":
                    {
                        sql = "UPDATE KeyWordsLib SET UpdateTS=@uts, [Stat]=" + EGenStat.Normal.GetInt32Str() + " WHERE UrlKey=@UK ";
                        break;
                    }
                case "del":
                    {
                        sql = "UPDATE KeyWordsLib SET UpdateTS=@uts, [Stat]=" + EGenStat.Delete.GetInt32Str() + " WHERE UrlKey=@UK ";
                        break;
                    }
                case "restore":
                    {
